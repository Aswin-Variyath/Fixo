...
@injectable()
export class NearbyTaskerSearchStore
    implements INearbyTaskerSearchStore
{
    async create(
        searchId: string,
        taskers: NearbyTaskerResponseDto[],
        session: NearbyTaskerSearchSession,
    ...
    async findById(
        searchId: string
    ): Promise<NearbyTaskerResponseDto[] | null> {
        ...
        return JSON.parse(
            value
        ) as NearbyTaskerResponseDto[];
    }

    async findSessionById(
        searchId: string
    ): Promise<NearbyTaskerSearchSession | null> {
        ...
        const session =
            JSON.parse(
                value
            ) as NearbyTaskerSearchSession;

        return {
            ...session,
            requestedDate: session.requestedDate
                ? new Date(session.requestedDate)
                : undefined,
        };
    }
    ...
    async deleteById(
        ...
        await redisClient.del(
            this.getResultKey(searchId)
        );

        await redisClient.del(
            this.getSessionKey(searchId)
        );
    }
